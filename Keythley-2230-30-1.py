...
names = ['Lab Model','Engineering Model', 'D-Orbit SDK']
...
#            inst = rm.open_resource(USB_PORT)
            ...
            for i in range(len(names)):
                ...
                current[i] = inst.query(f'MEASURE:CURRENT?').rstrip()   # Example: Replace with actual reading logic
                status[i] = inst.query(f'CHANNEL:OUTPUT?').rstrip() # Replace
...
#        grid_width = width // 2
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                stdscr.addstr(start_y + 6, start_x + 1, f"Status:  {'ON' if status[i * cols + j] else 'OFF'}")
                stdscr.attroff(curses.color_pair(color_pair))
                stdscr.addstr(start_y + 8, start_x + 1, f"Press {chr(ord('1')+i*cols+j)} to toggle ON/OFF".center(grid_width-2))
